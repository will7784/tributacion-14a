...
def get_export_path(db_name, filename):
    """Determina la carpeta (Kame) para el proyecto especializado y asegura que exista."""
    erp = "Kame"
...
def export_report_to_excel(db_name, texto_busqueda, tipo_analisis="inteligente"):
    ...
    COALESCE(
        ...
        (
            SELECT l2.fecha
            FROM ledger_runtime l2
            WHERE l2.comprobante = l.comprobante
    ...
    if tipo_analisis == "comprobante":
        ...
        SELECT 
            {sql_fecha} AS fecha,
            l.comprobante, l.cuenta, l.nombre_cuenta, l.ficha, l.razon_social, l.documento, l.vencimiento, l.concepto,
            debe, haber, (debe - haber) AS saldo
        FROM ledger_runtime l
        WHERE ({where_clause})
        ORDER BY l.comprobante, l.cuenta;
        """
        query = query.format(sql_fecha=sql_fecha)
        ...
        SELECT 
            {sql_fecha} AS fecha,
            l.comprobante, l.cuenta, l.nombre_cuenta, l.ficha, l.razon_social, l.documento, l.vencimiento,
        ...
        ORDER BY fecha, l.comprobante, l.cuenta, l.razon_social;
        """
        query = query.format(sql_fecha=sql_fecha)
    else:
    ...
    if df.empty:
        return None
    for col in ["ficha", "razon_social", "documento", "vencimiento", "concepto", "nombre_cuenta"]:
        if col in df.columns:
            df[col] = (
                df[col]
                .fillna("")
                .astype(str)
                .replace({"None": "", "nan": "", "NaT": "", "NAT": ""})
            )
        
    from pathlib import Path
    db_id = Path(db_name).stem.replace("_kame", "")
    fecha_str = datetime.now().strftime("%d%m%Y")
    clean_search = re.sub(r'[\\/*?:"<>|]', '_', texto_busqueda.lower().replace(',', '_'))[:30]
    nombre_base = f"analisis_{clean_search}_{tipo_analisis}_{fecha_str}_{db_id}.xlsx"
    ...
    try:
        ...
        f_min_df = run_query("SELECT MIN(fecha) as min_f FROM ledger_runtime", db_name)
    ...
    with pd.ExcelWriter(nombre_archivo, engine="xlsxwriter") as writer:
        ...
        for col in ["fecha", "vencimiento"]:
            if col in df.columns:
                ...
                # Fallback final para ISO texto no parseado en casos atípicos.
        ...
        # --- Inmovilizar Paneles ---
        # Detalle: inmovilizar en E7 (fila 7, columna E).
        ws_det.freeze_panes(6, 4)
        # Inmovilizar para que el primer dato visible desplazable sea B7.
        ws_piv.freeze_panes(6, 1)
...
def export_libro_mayor(db_name, texto_busqueda):
    ...
    with pd.ExcelWriter(nombre_archivo, engine="xlsxwriter") as writer:
        ...
        # Definir Columnas
        headers = ["Comprobante", "Fecha", "Ficha/RUT", "Razón Social", "Documento", "Concepto/Glosa", "Debe", "Haber", "Saldo"]
        col_widths = [12, 12, 15, 25, 18, 50, 15, 15, 15]
        ...
        for account in cuentas:
            ...
            ws.merge_range(current_row, 0, current_row, 8, f"CUENTA: {account} {account_name}", f_header_acc)
            current_row += 1
...
def export_pendientes_cuenta(db_name, cuenta, nombre_cuenta, fecha_balance):
    """Excel con documentos pendientes por cuenta (saldo por auxiliar y referencia de documento)."""
    query = """
    SELECT
        ficha AS [Auxiliar],
        razon_social,
    ...
    """
    df = run_query(query, db_name, params=[cuenta, fecha_balance])
    if df is None or df.empty:
        return None

    db_id = Path(db_name).stem.replace("_kame", "")
    f_emision = datetime.now().strftime("%d%m%Y")
    f_bal_clean = fecha_balance.replace("-", "")
    if "-" in fecha_balance:
        f_parts = fecha_balance.split("-")
        if len(f_parts) == 3 and len(f_parts[0]) == 4:
            f_bal_clean = f"{f_parts[2]}{f_parts[1]}{f_parts[0]}"

    nombre_base = f"pendientes_{cuenta} - {nombre_cuenta}_{f_bal_clean}_{f_emision}_{db_id}.xlsx"
    ...
    with pd.ExcelWriter(nombre_archivo, engine="xlsxwriter") as writer:
        df.to_excel(writer, sheet_name="Pendientes", index=False, startrow=6, header=False)
        workbook, ws = writer.book, writer.sheets["Pendientes"]
        f_corp = workbook.add_format({"bold": True, "font_size": 14, "align": "center"})
        f_info = workbook.add_format({"bold": True, "font_size": 10, "align": "center"})
        ws.merge_range("A1:D1", f"DOCUMENTOS PENDIENTES - CUENTA {cuenta}", f_corp)
        ws.merge_range("A2:D2", f"EMPRESA: {empresa_nombre} ({empresa_rut})", f_info)
        ws.merge_range("A3:D3", f"CUENTA: {nombre_cuenta}", f_info)
        ws.merge_range(
            "A4:D4",
            f"FECHA CORTE: {fecha_balance} | EMISIÓN: {datetime.now().strftime('%d-%m-%Y')}",
            f_info,
        )
        ...
        for i, col in enumerate(df.columns):
            ws.write(5, i, col, f_header)
            ws.set_column(i, i, 25)
        for r in range(len(df)):
            for c in range(len(df.columns)):
                ws.write(r + 6, c, df.iloc[r, c], f_num if c == 3 else f_text)
    return nombre_archivo


def export_dataframe_simple_excel(db_name, df, filename_base, sheet_name="data"):
    """Exportador simple de DataFrame para vistas de soporte (notas/relaciones)."""
    if df is None or df.empty:
        return None
    safe_base = re.sub(r'[\\/*?:"<>|]', "_", str(filename_base))
    out_path = get_export_path(db_name, f"{safe_base}.xlsx")
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
        df.to_excel(writer, index=False, sheet_name=sheet_name)
        ws = writer.sheets[sheet_name]
        for i, col in enumerate(df.columns):
            max_len = max(len(str(col)), int(df[col].astype(str).str.len().max() if len(df) else 0))
            ws.set_column(i, i, min(max(max_len + 2, 12), 50))
    return out_path


def export_informe_gestion_excel(db_name, filename_base, sheets: List[Tuple[str, pd.DataFrame]]):
    """
    Exporta uno o más DataFrames a un .xlsx (hojas nombradas).
    Aplica formato numérico chileno (miles con punto) en columnas típicas de monto.
    """
    if not sheets:
        return None
    safe_base = re.sub(r'[\\/*?:"<>|]', "_", str(filename_base))
    out_path = get_export_path(db_name, f"{safe_base}.xlsx")
    money_cols = {"acumulado", "monto", "saldo", "debe", "haber"}
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
        workbook = writer.book
        num_fmt = workbook.add_format({"num_format": "#.##0", "align": "right"})
        head_fmt = workbook.add_format({"bold": True, "bg_color": "#34495E", "font_color": "white"})
        for sheet_name, df in sheets:
            if df is None or df.empty:
                continue
            sn = sheet_name[:31] if len(sheet_name) > 31 else sheet_name
            df.to_excel(writer, index=False, sheet_name=sn)
            ws = writer.sheets[sn]
            for i, col in enumerate(df.columns):
                cname = str(col).strip().lower()
                max_len = max(len(str(col)), int(df[col].astype(str).str.len().max() if len(df) else 0))
                ws.set_column(i, i, min(max(max_len + 2, 14), 55))
                if cname in money_cols or cname.startswith("monto") or cname.startswith("saldo"):
                    ws.set_column(i, i, min(max(max_len + 2, 16), 55), num_fmt)
            # Reescribe cabecera con formato
            for c in range(len(df.columns)):
                ws.write(0, c, df.columns[c], head_fmt)
    return out_path
